...
export const filterBySeverity = (warningData, minimumSeverity) =>
    warningData.warnings.filter(warning => warning.severity >= minimumSeverity);

export const filterSinceLastUpdate = (oldWarnings, newWarnings) =>
    newWarnings.filter(newWarning => !oldWarnings.some(oldWarning => getEqualityResult(newWarning, oldWarning)));
